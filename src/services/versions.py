...
async def _get_card_data(card_number: int, client: httpx.AsyncClient) -> dict | None:
    ...
    logger.debug(f'Iniciando busca de dados para o card {card_number}')
    url = f'{URL_API}/{card_number}'
    try:
        response = await client.get(url)
        response.raise_for_status()
        data = response.json()
        logger.debug(f'Dados do card {card_number} obtidos com sucesso')
        return {'CD_CARD': data['id'], 'SITE_VERS': data['versao']}
    except httpx.HTTPStatusError as e:
        logger.error(f'Erro ao buscar card {card_number}: {e.response.status_code} - {e.response.text}')
    except httpx.RequestError as e:
        logger.error(f'Erro de conexão ao buscar card {card_number}: {e!s}')
    return None
...
async def _get_all_cards_data(card_numbers: list[int], client: httpx.AsyncClient) -> list[dict]:
    ...
    logger.info(f'Iniciando busca de dados para {len(card_numbers)} cards')
    tasks = [_get_card_data(card, client) for card in card_numbers]
    site_data = await asyncio.gather(*tasks)
    logger.info(f'Busca de dados para {len(card_numbers)} cards concluída')
...
async def _compare_versions(db_df: pd.DataFrame, client: httpx.AsyncClient) -> pd.DataFrame:
    ...
    logger.info('Iniciando comparação de versões')
    try:
        site_data = await _get_all_cards_data(db_df['CD_CARD'].tolist(), client)
        if not site_data:
            logger.warning('Nenhum dado válido encontrado no site.')
            return pd.DataFrame()

        site_df = pd.DataFrame(site_data)
        logger.info('Comparação de versões concluída com sucesso')
        return pd.merge(db_df, site_df, on='CD_CARD')
    except Exception as e:
        logger.error(f'Erro ao comparar versões: {e!s}')
        return pd.DataFrame()
...
async def _get_all_info_cards_data(session: Session, client: httpx.AsyncClient) -> list[dict]:
    ...
    logger.info('Iniciando processamento de InfoCards')
    try:
        ...
        if compared_df.empty:
            logger.warning('Nenhuma comparação de versões disponível.')
            return []
        ...
        logger.info('Processamento de InfoCards concluído com sucesso')
        return ic_df.to_dict(orient='records')
    except Exception as e:
        logger.error(f'Erro ao processar InfoCards: {e!s}')
        return []


async def get_all_versions_data(session: Session, token: str) -> dict | None:
    """Busca dados de versões de cartões."""
    logger.info('Iniciando busca de dados de versões de cartões')
    try:
        ...
        logger.info('Busca de dados de versões de cartões concluída com sucesso')
        return {'versoes': data, 'exec': datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    except httpx.RequestError as e:
        logger.error(f'Erro de conexão ao buscar dados: {e!s}')
    except Exception as e:
        logger.error(f'Erro inesperado ao buscar dados: {e!s}')
    return None