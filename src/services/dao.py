...
def get_ocr_meta(session: Session) -> list[dict]:
    ...
    query = text(
        """
        SELECT VL_META_IN_MBZ as "Meta conexão", VL_META_CARD AS "Meta infocard",
        CD_PRF_CARD AS "Prefixo", CD_IND_ATB AS "Código indicador",
...
def get_regua_cards(session: Session) -> list[dict]:
    ...
    query = text(
        """
        SELECT RGUA_ATB AS "Régua Conexão", RGUA_CARD AS "Régua infocard",
        CD_PRF_CARD AS "Prefixo", CD_IND_ATB AS "Código indicador",
        NM_IN_MBZ AS "Nome indicador"
        FROM DB2ATB.VS_DVGA_ATB_CARD
        WHERE OCR_RGUA = 1
...
def update_card_version(session: Session, params: dict) -> bool:
    """Atualiza o versionamento de um cartão."""
    query = text(
        """
        UPDATE DB2ATB.INFO_CARDS SET NM_CARD = :name, VL_RGUA_MAX_CARD_CLTO = :vl,
        QT_PTO_FXA_RGUA_CARD_CLTO = :qt, VL_META_CARD = :meta, CD_VERS = :version
        WHERE CD_CARD = :cd
        """
    )
